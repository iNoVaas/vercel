...
export default async function ShopPage({
  ...
  let allCatsForCount: { category: string }[] = [];
  try {
    allCatsForCount = await prisma.product.findMany({
      select: { category: true },
    });
  } catch {
    const sampleData = (await import("@/db/sample-data")).default;
    allCatsForCount = sampleData.products.map((p) => ({ category: p.category }));
  }
